...
import { Score } from "./includes/types.js";
...
const start = async () => {
    ...
    taWS.stateManager.on("matchUpdated", ([match, tournament]) => {
        if (mode === "BR") {
            if (typeof match.selectedMap !== "undefined") {
    ...
    taWS.stateManager.on("matchDeleted", ([match, _]) => {
        ws.send(JSON.stringify({ Type: "2", message: match }, (_, value) =>
            typeof value === 'bigint'
                ? value.toString()
                : value // return everything else unchanged
        ));
